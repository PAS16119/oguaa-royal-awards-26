...
async function stateConfirmCandidate(input, session, cfg) {
  ...
  if (input !== '1') {
    return con(`${session.data.candidateName} — ${session.data.award}\n1. Confirm\n2. Back (wrong code)`);
  }
...
async function stateAwaitVotes(input, session, cfg) {
  ...
  return {
    ...
    message: con(`GHS ${amountGHS} — ${votes} vote(s) for ${session.data.candidateName} (${session.data.award})\n1. Confirm to complete\n2. Cancel`),
  };
}

async function stateConfirm(input, session, cfg) {
  if (input === '2') return end('Cancelled. No payment was made.');
  if (input !== '1') return con(`1. Confirm to complete\n2. Cancel`);
